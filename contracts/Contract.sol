...
contract SocialMedia{
    ...
    mapping (string => bool) isUserNameUsed;
    constructor(){
    ...
    function createUser(string memory _name,string memory _mail,string memory _password)public returns(bool){
        require((!isRegistered[_mail])&&(!isUserNameUsed[_name]), 'This e-mail is already registered');
        users.push(User(_name,_mail,_password));
        uid[_mail] = NoOfUsers;
        isRegistered[_mail] = true;
        isUserNameUsed[_name] = true;
        NoOfUsers+=1;
        return true;
    }

    function validUser(string memory _mail,string memory _password)public view returns(bool,User memory){
        if(uid[_mail]!=0){
            string memory pass = users[uid[_mail]-1].password;
            if(keccak256(abi.encodePacked(pass)) == keccak256(abi.encodePacked(_password))){
                return (true,users[uid[_mail]-1]);
            }
        }
        return (false,users[uid[_mail]-1]);
    }
